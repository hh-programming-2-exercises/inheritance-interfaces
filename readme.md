...
This assignment repository contains a set of Java exercises to help you get acquainted with inheritance and interfaces. To support the exercises, we recommend the following materials:
...
```sh
git clone https://github.com/YOUR_GITHUB_USERNAME/programming-2-inheritance-interfaces.git
```
...
Further develop the [`Product`](./src/main/java/inheritance/webshop/Product.java) class from the previous part of this assignment so that it also implements the [`MarkdownExport`](./src/main/java/interfaces/markdown/MarkdownExport.java) interface. For products, the markdown representation must include the product's name, description, and price, but you can otherwise define the string format freely.
...
* [`Main`](./src/main/java/dependency_injection/Main.java). *The main task instructions can be found in this class. This is the only class that needs to be modified in this part*.
